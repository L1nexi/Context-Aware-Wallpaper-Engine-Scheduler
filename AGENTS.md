...
- 测试不是一种仪式。测试的目的是用来验证逻辑。如果对于简单直接的逻辑，你应该做的是正面验证逻辑的完全性。对于复杂的逻辑，你应该做的是覆盖边界条件和潜在的误用场景。不要把测试滥用为“为了测试而测试”，或者“为了覆盖率数字而测试”。如果你发现自己在写一个测试用例时，感觉它没有增加任何新的验证价值，那么它很可能就是不必要的。
