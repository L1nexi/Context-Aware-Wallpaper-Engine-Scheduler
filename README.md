...
- **防打扰**：仅在用户空闲时切换壁纸，区分“播单切换”与“播单内轮播”。支持独立冷却时间，支持强制切换。
...
2. **Wallpaper Engine 配置**:
   - 在 Wallpaper Engine 中创建并命名多个播放列表，推荐每一个播放列表对应一种“场景”，如“工作”、“放松”、“雨天”、“暖冬”等。
   - 建议将播放列表的**轮播设置/计时器**设置为**从不**。本程序会接管壁纸切换逻辑（包括播单内的壁纸轮播），若 WE 自身也在自动轮播，可能会导致壁纸频繁变动或逻辑冲突。
3. **修改关键参数**:
   - `we_path`: 指向你本地的 Wallpaper Engine 路径（例如 `...\\wallpaper64.exe`）。
   - `playlists`: 填入你在 WE 中定义的播放列表名称。
     - `tags`: 为每个播放列表定义触发标签及其权重。
     - > **请注意，歌单配置中起作用的是各标签的占比而非绝对值大小。**举例来说，{ "#focus": 0.9,"#chill": 0.2,} 和 { "#focus": 9,"#chill": 2,} 在匹配时具有相同的表现。
   ...
   - `disturbance`: 调整切换频率（支持“播单切换”与“壁纸轮播”独立计时）。
     ...
     - `switch_on_start`: 是否在程序启动时立即切换壁纸（默认关闭）。
   - `policies/weather` (推荐) 填入 OpenWeatherMap API Key 以启用天气策略。
