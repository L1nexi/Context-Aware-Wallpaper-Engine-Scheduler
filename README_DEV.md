...
1.  **Policy (策略插件):** 独立的逻辑单元，负责观察特定维度的上下文，并输出带有权重的标签建议。
    *   `ActivityPolicy`: "过去一小时都在写代码" -> 建议 `#work (0.8)`
...
```json
{
  ...
  "policies": {
    "activity": { "enabled": true, "weight_scale": 1.0 },
    "season": { "enabled": true, "weight_scale": 0.5 }, // 季节权重较低，作为背景色
    "mood": { "enabled": true, "weight_scale": 2.0 }    // 心情权重极高，可覆盖其他
  }
}
```
