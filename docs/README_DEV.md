...
> **⚠️ 文档状态：部分历史存档**
>
> 本文档写于早期设计阶段，随项目演进已部分失效（Section 2、3 尤甚）。
> **当前架构以下列来源为准，本文档不再主动维护：**
>
> | 内容               | 权威来源                          |
> | ------------------ | --------------------------------- |
> | 当前架构与接口约定 | `.github/copilot-instructions.md` |
> | 已知问题与决策记录 | `docs/TODOS.md`                   |
> | 未来开发方向       | `docs/ROADMAP.md`                 |
>
> Section 4–12 仍基本准确，可作参考。Section 2（系统设计）和 Section 3（路线图）保留为**历史记录**，不反映当前实现。
...
## 2. 系统设计文档 (SDD) `[HISTORICAL — 参见 copilot-instructions.md]`
...
- **Actuator (执行器):**
  ...
  - **逻辑:**
    ...
    - 咨询 `SchedulingController` 是否允许操作。
...
2. **Matcher (匹配器):** 负责后处理（降级、聚合），并在配置表中寻找最佳 Playlist。
   - **匹配算法 (Cosine Similarity):**
     ...
     - **降级策略 (Fallback):** 当 Policy 输出的标签不在任何播放列表中时，Matcher 会根据配置的 `fallback` 规则递归展开，尝试找到相关标签进行匹配。
...
详见 `scheduler_config.example.json` 示例文件。

### 2.5 防打扰与调度策略 (Disturbance Control) [outdated]
...
## 3. 开发路线图 (Roadmap) `[HISTORICAL — Phase 1–5 均已完成，未来方向见 ROADMAP.md]`
...
### Phase 5: 感知增强与工程化 (v1.3.0) - [WIP]

- **目标:** 增强感知层与控制层，提升调度质量与可维护性。
- **任务:**
  1. **D: WeatherSensor 拆分** — 将 WeatherPolicy 内的 HTTP 请求逻辑拆分为独立 Sensor，通过 context 共享天气数据（含 sunrise/sunset）。
  2. **E: TimePolicy 动态峰值** — 读取 context 中 OWM 返回的 sunrise/sunset，动态计算 day_start/night_start，替代固定配置。
  3. **Gate 重构** — 将 Controller 中的 CPU/全屏门控逻辑封装为 `CpuGate`/`FullscreenGate` 类，保持线性 if-chain 但增强可测试性。
  4. **A: FullscreenSensor + 全屏门控** — `SHQueryUserNotificationState` 检测全屏/演示模式，defer 而非 block。
  5. **B: 配置热重载** — `os.path.getmtime()` 检测配置文件变更，自动重载 Policies/Matcher/Controller。
  6. **C: 操作统计** — Actuator 在实际切换时写入 `history.jsonl`，记录时间戳、事件类型、播放列表和上下文快照。
...
| Policy         | 负责的标签域                                            |
| -------------- | ------------------------------------------------------- |
| ActivityPolicy | `#focus`, `#chill`, `#game`, 等自定义活动标签           |
...
Controller 中的 CPU 门控、全屏门控等 `if` 逻辑在 `can_switch_playlist` 和 `can_cycle_wallpaper` 中重复出现。将每种门控封装为独立类，保持线性 if-chain 的可读性，同时提升可测试性和可扩展性。

### 6.2 Gate 接口

Gate 不使用 ABC——当前只有 2 个实现，抽象基类属于过度设计。每个 Gate 是一个简单类，暴露 `should_defer(context) → bool`：

```python
class CpuGate:
    def __init__(self, threshold: float):       # 0 = disabled
    def should_defer(self, context) -> bool: ...
...
```mermaid
cooldown → [Gate₁, Gate₂, ...] → idle/force
```

Controller 在 `__init__` 中根据配置构建 `_gates: list`，在冷却检查之后、idle/force 判断之前依次调用。任一 gate 返回 `True` 即 defer（不重置计时器）。

---

## 7. WeatherSensor 与数据共享 (v1.3.0)

### 7.1 拆分动机

WeatherPolicy 内嵌 HTTP 请求逻辑，导致：

1. TimePolicy 无法读取 OWM 返回的 `sunrise`/`sunset` 数据。
2. 网络调用混入 Policy 的纯计算逻辑中。

**原则: 跨 Policy 数据共享必须通过 context。**

### 7.2 WeatherSensor

位于 `core/sensors.py`。行为：

- 以 `interval` 秒为周期调用 OWM 2.5 `/weather` API。
- 返回值写入 `context["weather"]`：

```python
{
    "id": 800,            # OWM 天气代码
    "main": "Clear",      # 天气主类
    "sunrise": 1719028800, # Unix timestamp (UTC)
    "sunset": 1719082800,  # Unix timestamp (UTC)
}
```

- WeatherPolicy.get_tags 从 `context["weather"]` 读取，不再自行发起 HTTP 请求。
- TimePolicy.get_tags 从 `context["weather"]` 读取 `sunrise`/`sunset` 动态计算峰值。

---

## 8. TimePolicy 动态日出日落 (v1.3.0)

当 `context["weather"]` 中包含 `sunrise`/`sunset` 时，TimePolicy 将 Unix 时间戳转换为本地小时，用作 `day_start`/`night_start` 动态值。若 WeatherSensor 未启用或数据缺失，回退到 config 中的静态配置。

峰值在每次 `get_tags()` 调用中按需更新（仅当日出日落值变化时重算）。

---

## 9. FullscreenSensor (v1.3.0)

使用 Win32 `SHQueryUserNotificationState` (shell32.dll)。检测以下状态视为"全屏"：

| 值  | 常量                           | 含义              |
| --- | ------------------------------ | ----------------- |
| 2   | `QUNS_BUSY`                    | 全屏应用正在运行  |
| 3   | `QUNS_RUNNING_D3D_FULL_SCREEN` | D3D 独占全屏      |
| 4   | `QUNS_PRESENTATION_MODE`       | 演示模式 (PPT 等) |

零配置，覆盖 3A 游戏、全屏视频、PPT 演示等场景。

---

## 10. 配置热重载 (v1.3.0)

Scheduler 在 `_run_loop` 每 tick 检查 `os.path.getmtime(config_path)`。若 mtime 变化：

1. 重新加载 JSON 配置。
2. 重建 Policies、Matcher（播放列表可能变化）、Controller、Sensor 参数。
3. 保留 `current_playlist`、暂停状态、线程状态。
4. 写入 INFO 日志。

无需托盘菜单触发，编辑配置文件后自动生效。

---

## 11. 操作统计 (v1.3.0)

Actuator 在每次实际执行操作时追加一行 JSON 至 `history.jsonl`（与 `state.json` 同目录）。

```json
{"ts": "2025-01-15T14:32:00", "event": "playlist_switch", "from": "NIGHT_CHILL", "to": "BRIGHT_FLOW", "tags": {"#focus": 0.8, "#day": 0.6}}
{"ts": "2025-01-15T15:02:00", "event": "wallpaper_cycle", "playlist": "BRIGHT_FLOW"}
```

文件仅追加写入，不做轮转（典型场景每天 < 50 行）。
...
"Pydantic 模型 → 自动生成配置 UI" 是一个有相当用户量的需求，但尚无同时满足「Windows 桌面 + Pydantic v2 + 零重型依赖」的开箱即用方案：

| 库                           | 适用场景                                 | 评估                   |
| ---------------------------- | ---------------------------------------- | ---------------------- |
| **`pydantic-settings`**      | 读取环境变量/`.env`，无 GUI              | ❌ 无 UI               |
| **`streamlit`**              | 数据应用快速原型，含侧边栏表单           | ⚠️ 浏览器界面 + 重依赖 |
| **`nicegui`**                | Python 驱动 Web UI，有 Pydantic 绑定辅助 | ⚠️ 需本地 Web server   |
| **`gradio`**                 | ML Demo 场景，支持 dataclass 自动表单    | ❌ 场景不符            |
| **`pydantic-form` (PyPI)**   | 专门生成 tkinter/HTML 表单，较小众       | ⚠️ 维护状态不明        |
| **手写 `model_fields` 遍历** | 自主可控，依赖归零                       | ✅ 推荐                |

**结论：** 最可控路径是自行遍历 `model_fields` 生成控件，代码量可控（单文件 ~200 行覆盖所有字段类型）。
...
### 12.4 部署形态：嵌入式 vs 独立运行

配置编辑器可以完全**脱离 Scheduler 主体独立运行**——它只需读写 `scheduler_config.json`，热重载由 Scheduler 侧处理，两者无直接通信。

| 形态                     | 实现方式                                       | 优缺点                                         |
| ------------------------ | ---------------------------------------------- | ---------------------------------------------- |
| **嵌入托盘**（推荐起步） | 托盘「设置...」菜单项启动 `ConfigEditorWindow` | 与主程序同进程，打包最简单                     |
| **独立 EXE**             | `config_editor.py` + PyInstaller 单独打包      | 无需启动 Scheduler 也能配置；双 EXE 分发略复杂 |
| **本地 Web 应用**        | `nicegui` + 浏览器                             | 界面最自由；需本地端口，有安全边界需注意       |
| **Windows 原生应用**     | WinUI3 / Python + WinRT                        | Fluent Design；跨语言维护成本高                |

**当前建议：** 先做嵌入托盘的 tkinter 版本，验证表单生成逻辑；有独立发布需求时将 `config_editor.py` 提取为独立入口即可。

### 12.5 实现方案选型

| 方案              | 优点                                 | 缺点                              |
| ----------------- | ------------------------------------ | --------------------------------- |
| **tkinter + ttk** | 零依赖，已在 tray.py 引入            | 原生控件丑陋，布局繁琐            |
| **ttkbootstrap**  | tkinter 超集，现代主题               | 多一个依赖，打包体积略增          |
| **nicegui**       | HTML/CSS 自由布局，Pydantic 绑定辅助 | 需本地 Web server，引入较重依赖   |
| **pywebview**     | 内嵌 Webview2，可用完整前端框架      | 需 `pywebview` 依赖 + JS 开发成本 |

**推荐路径：** 先用原生 `tkinter` + `ttk` 验证逻辑（与 tray.py 共用同一 tk 根），有美化需求时再引入 `ttkbootstrap` 或换 `nicegui`。

### 12.6 热重载集成

GUI 保存配置后只需写入 `scheduler_config.json`——调度器热重载机制（`_hot_reload`，每 tick 检查 `getmtime`）自动检测变更并无缝重建运行时组件，**无需 GUI 与调度器之间的直接通信**。
...
选项 A — **ttkbootstrap**：改造现有 tkinter 窗口，引入现代主题，改动最小。  
选项 B — **nicegui**：重写为本地 Web 应用；可同时作为独立 EXE 发布，无需启动 Scheduler 也能配置。  
选项 C — **独立 EXE**：将 `config_editor.py` 提取为独立 PyInstaller 入口，不绑定 tray。

触发条件：阶段一/二完成且有具体美化/分发需求时再评估。
