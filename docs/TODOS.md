...
> 未来功能方向（Dashboard / 自动 Tag / history 消费）详见 [`docs/ROADMAP.md`](ROADMAP.md)。
...
| P1-B | Policy 输出保模长降维（去无效分量保总模长） |   中   | #4   | Abolished(Turned to Fallback) |
...
## 未来功能（Feature Roadmap）

以下条目超出 bug-fix / 架构修复范围，已移至 [`docs/ROADMAP.md`](ROADMAP.md) 详细规划。此处仅作索引。

| ID  | 功能                          | 摘要                                                      | 状态 |
| --- | ----------------------------- | --------------------------------------------------------- | :--: |
| R1  | 系统状态 Dashboard            | 实时展示 aggregated_tags / similarity；Observer hook 推送 |  📋  |
| R2  | 自动播单 Tag 生成（离线脚本） | 解析 workshop `project.json`；三层方案（映射/文字/CLIP）  |  📋  |
| R3  | history.jsonl 时间轴可视化    | 切换时间轴 + 调优辅助；建议集成进 R1 Dashboard            |  📋  |
