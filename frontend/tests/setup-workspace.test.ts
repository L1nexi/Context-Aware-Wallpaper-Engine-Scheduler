...
test("saving shows a server field error in its owning setup step", async () => {
  ...
  const reviewButton = wrapper.findAll("button").find((button) => button.text().includes("Review and finish"))
  ...
  expect(wrapper.get("section h1").text()).toBe("Connect weather")
  expect(wrapper.get("#weather-api-key").attributes("aria-invalid")).toBe("true")
  expect(wrapper.text()).toContain("OpenWeatherMap rejected this API key")
})

test("first-run setup cannot advance before Wallpaper Engine is ready", async () => {
  api.scanPlaylists.mockResolvedValue({
  ...
  const nextButton = wrapper.findAll("button").find((button) => button.text().trim() === "Continue")
  expect(nextButton).toBeDefined()
  await nextButton!.trigger("click")

  expect(wrapper.get("section h1").text()).toBe("Connect Wallpaper Engine")
  expect(wrapper.text()).toContain("Complete the required fields on this page first")
  expect(wrapper.get("#wallpaper-engine-path").attributes("aria-invalid")).toBe("true")
})
...
test("editing one step keeps server errors that belong to another step", async () => {
  ...
  await wrapper.findAll("button").find((button) => button.text().includes("Review and finish"))!.trigger("click")
  ...
  expect(wrapper.get("section h1").text()).toBe("Recognize activity")
  expect(wrapper.text()).toContain("Activity targets conflict")
  expect(wrapper.text()).not.toContain("OpenWeatherMap rejected this API key")
})
